  ...
  describe('clean method', () => {
    describe('with string URLs', () => {
      ...
      it('should clean Facebook tracking parameters', () => {
        const dirtyUrl = 'https://facebook.com/page?fbclid=abc123&utm_source=social';
        const cleanedUrl = linkCleaner.clean(dirtyUrl);
      ...
      it('should clean Amazon tracking parameters', () => {
        ...
        expect(cleanedUrl.toString()).toBe('https://amazon.com/product?other=param');
      });

      it('should preserve exception parameters', () => {
        const dirtyUrl = 'https://google.com/search?q=important&gclid=123&utm_source=email';
        const cleanedUrl = linkCleaner.clean(dirtyUrl);

        expect(cleanedUrl.toString()).toBe('https://google.com/search?q=important');
      ...
      it('should handle referral parameters', () => {
        ...
        expect(cleanedUrl.toString()).toBe('https://google.com/page?other=param');
      });
      ...
      it('should handle empty query parameters', () => {
        const dirtyUrl = 'https://google.com/search?gclid=&utm_source=email&q=test';
        const cleanedUrl = linkCleaner.clean(dirtyUrl);
    ...
    describe('with URL objects', () => {
      ...
      it('should return original URL object if no provider matches', () => {
        ...
        expect(cleanedUrl.toString()).toBe('https://example.com/page?param=value');
      });
    });

    describe('redirection handling', () => {
      it('should follow redirection parameters', () => {
        const redirectUrl = encodeURIComponent('https://example.com/target?param=value');
        const dirtyUrl = `https://google.com/url?url=${redirectUrl}&gclid=123`;
        const cleanedUrl = linkCleaner.clean(dirtyUrl);

        expect(cleanedUrl.toString()).toBe('https://example.com/target?param=value');
      });
      ...
      it('should handle invalid redirection URLs gracefully', () => {
        ...
        expect(cleanedUrl.toString()).toBe('https://google.com/url?url=invalid-url');
      });
    ...
    describe('exception handling', () => {
      it('should preserve parameters matching exception patterns', () => {
        ...
        expect(cleanedUrl.toString()).toBe('https://tracking-domain.com/page?important_param=keep');
      });
    });
    ...
    describe('error handling', () => {
      ...
      it('should handle exception patterns with invalid regex', () => {
        ...
        const cleanerWithBadException = new LinkCleaner(rulesWithInvalidException);
        const consoleSpy = jest.spyOn(console, 'error').mockImplementation(() => {});
    ...
    describe('complex scenarios', () => {
      it('should handle multiple tracking parameters', () => {
        ...
        expect(cleanedUrl.toString()).toBe('https://google.com/search?q=test&other=keep');
      });

      it('should handle URLs with fragments', () => {
        const urlWithFragment = 'https://google.com/page?gclid=123&q=test#section';
        const cleanedUrl = linkCleaner.clean(urlWithFragment);

        expect(cleanedUrl.toString()).toBe('https://google.com/page?q=test#section');
      });
      ...
      it('should handle URLs with ports', () => {
        ...
        expect(cleanedUrl.toString()).toBe('https://google.com:8080/page?q=test');
      });
